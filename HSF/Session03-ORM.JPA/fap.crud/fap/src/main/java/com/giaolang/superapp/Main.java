...
public class Main {
    ...
    public static void update() {
        ...
        //em.remove(x);
        //em.remove(y);
        x.setSalary(25_000_000);  //25 triệu
        y.setGpa(9.2);
    ...
    //TA PHẢI NHÉ NÓ VÀO TRANSACTION ĐỂ THEO DÕI: HOẶC TẤT CẢ, HOẶC KO GÌ CẢ
    ...
    public static void remove() {
        ...
        Student y = em.find(Student.class, "SE2");  //ID STUDENT LÀ CHUỖI
        em.remove(x);
        em.remove(y);
        em.getTransaction().commit();  //khoá sổ 2 hành động
    ...
    //EntityManager là ông sếp quản lí các Entity ~ chính là các class có @Entity và quản lí các object tạo từ class Entity: sếp có thể thêm persit(); xoá remove(); cập nhật merge(); tìm theo PK find(): lí do có hàm tìm theo key, vì ta luôn có nhu cầu thao tác trên 1 dòng/row/record cụ thể trong table
    //SWP admin, có màn hình quản lí user, phân loại, show table có nhiều dòng, phân trang, filter, cuối dòng có cột Actions: Update | Delete -> xử lí đúng 1 dòng đang select -> theo PK
    ...
    public static void findById() {
        ...
        System.out.println("Lecturer info: " + x); //gọi thầm tên em toString()
        System.out.println("Student info: " + y); //gọi thầm tên em toString()
    ...
    //setParameter("pName", "%AN%").setParameter(...)
