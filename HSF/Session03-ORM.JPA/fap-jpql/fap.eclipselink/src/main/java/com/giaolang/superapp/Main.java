...
public class Main {
    ...
    private static EntityManagerFactory emf = Persistence.createEntityManagerFactory("com.giaolang.superapp.eclipselink-PU");
    ...
    // khi làm các hành động xóa sửa thêm -> ảnh hưởng đến sự thay đổi database
    // ta phải nhét nó vào transaction để theo dõi: hoặc tất cả, hoặc không gì cả
    // Nguyên lý DO ALL OR NOTHING: ACID

    public static void remove (){
        EntityManager em = emf.createEntityManager();
        em.getTransaction().begin();
        // Tìm theo key thì chắc chắn chỉ trả ra 1 dòng
        Lecturer x = em.find(Lecturer.class, 1); // Giá trị PK muố ntimf
        Student y = em.find(Student.class, "SE10"); // ID của Student là chuỗi
        em.remove(x);
        em.remove(y);
    ...
    public static void update (){
        ...
        em.getTransaction().begin();
        // Tìm theo key thì chắc chắn chỉ trả ra 1 dòng
        Lecturer x = em.find(Lecturer.class, 5); // Giá trị PK muố ntimf
        Student y = em.find(Student.class, "SE3"); // ID của Student là chuỗi
    ...
    //EntityManager là ông sếp quản lí các Entity ~ chính là các class có từ khóa @Entity và quản lí các object được tạo từ Entity sếp có thể thêm persit(), xóa remove(), update merge, tìm theo PK find(): lí do có hàm tìm theo key, vì ta luôn có yêu cầu thao thác trên 1 dòng/row/record cụ thể trong table

    //SWP admin, có màn hình quản lí user, phân loại sản phẩm, show table nhiều dòng, phân trang, filter, cuối dòng có cột Actions: Update | Delete -> xử lí đúng 1 dòng đang select -> theo PK
    ...
    public static void findById (){
        ...
        System.out.println("Lecture info: " + x);
    ...
    // jpql: select x from lecturer x where.salary = :pSalary
    ...
    //                                              LIKE 'AN%' -- TÊN BẮT ĐẦU BẰNG CHỮ AN
    //PSQL: SELECT lec FROM Lecturer lec WHERE lec.name LIKE :pName AND ...
    //setParameter("pName", "%AN%").setParameter(...).getResultList()
...
//        List<Lecturer> result = em.createQuery("SELECT x FROM Lecturer x WHERE x.salary = :pSalary ", Lecturer.class).setParameter("pSalary", 20000000).getResultList();
        ...
        // nếu hàm trả về object ta có quền chấm tiếp thay vì khai báo biên để hứng sau đ biến chấm tiếp.
...
//        });
    ...
    public static void insertLecturers() {
        ...
        Lecturer an = new Lecturer("AN NGUYỄN", 1990, 20000000);
        Lecturer binh = new Lecturer("BÌNH LÊ", 1991, 20000001);
...
//        EntityManagerFactory emf = Persistence.createEntityManagerFactory("com.giaolang.superapp-PU"); //gửi thông số cấu hình Server, nhà thầu JPA: Hibernate, nhà thầu JDBC cho JPA class lo để tạo kết nối tới CSDL cụ thể SQL Server
    ...
    public static void getAllStudents() {
        ...
        EntityManager em = emf.createEntityManager();
        List<Student> result = em.createQuery("SELECT s FROM Student s", Student.class).getResultList();  //HẬU TRƯỜNG LÀ SELECT * FROM STUDENT SQL TRUYỀN THỐNG - SẼ IN XEM NẾU KHAI BÁO CẤU HÌNH TRONG FILE .XML
